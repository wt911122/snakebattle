...
;(function(game, cjs){
	...
	game.snake = function(config){
				...
				if(Snakes[one] !== this && !Snakes[one].isDead){
					...
					while(itr.hasNext()){
						var part = itr.next(),
							d_x = part.x - _head.x,
							d_y = part.y - _head.y;
						if(Math.abs(d_x) < _checkCrashBoudary && Math.abs(d_y) < _checkCrashBoudary){
		...
		function _turnTo(degree){
			...
			_toDirection = (degree + 360)%360;
			var span = degree - _direction;
		...
		function _eat(){
			...
			while(pieces[idx]){
				c = pieces[idx];
				if(Math.abs(c.x - _coords.x) < 10 && Math.abs(c.y - _coords.y) < 10){
					c.visible = false;
					_score += c.score;
					pieces.splice(idx, 1);
					idx--;
		...
		constructor.prototype = {
			...
			set MODE(value){
				...
				if(obj){
					...
					if(obj.eat) this._eat = obj.eat;
					if(obj.checkCrash) this._checkCrash = obj.checkCrash;
				}
			}
		}
		...
		constructor.prototype.extend = function(extendsObj){
			var name = extendsObj.name;
			if(_MODE_LIB.hasOwnProperty(name)) throw "mode already defined";
			_MODE_LIB[name] = extendsObj;
		}
